...
/**
 * Pantalla de Batalla Multijugador - Lujuria (Piso 4)
 */
public class PantallaLujuriaMulti implements Screen {
    ...
    @Override
    public void show() {
        System.out.println("PantallaLujuriaMulti.show() ejecutado");
        ...
        if (hiloCliente == null || !hiloCliente.isConectado()) {
            System.out.println("PantallaLujuriaMulti: ERROR - No hay cliente conectado");
            estadoActual = EstadoBatallaMulti.ESPERANDO_CONEXION;
        } else {
            System.out.println("PantallaLujuriaMulti: Cliente conectado, esperando datos de batalla");
        ...
        if (Config.personajeSeleccionado == null) {
            System.out.println("PantallaLujuriaMulti: WARN - PersonajeSeleccionado es NULL.");
    ...
    private void parsearDatosBatalla(String datos) {
        ...
        try {
            ...
            // if (nivelRecibido < Config.nivel) return; // FIX: Permitir reinicio de nivel en nuevo piso
    ...
    private Imagen crearSpriteEnemigo(String nombre) {
        String recurso = Recursos.ENEMIGOLUJURIA1; // Default: Onseno
        
        if (nombre.equalsIgnoreCase("Onseno")) {
            recurso = Recursos.ENEMIGOLUJURIA1;
        } else if (nombre.equalsIgnoreCase("Mascara")) {
            recurso = Recursos.ENEMIGOLUJURIA2;
        } else if (nombre.equalsIgnoreCase("Jujurion")) {
            recurso = Recursos.MINIBOSSLUJURIA;
        }
        
        System.out.println("DEBUG SPRITE LUJURIA: '" + nombre + "' -> " + recurso);
    ...
    private boolean actualizarEstadoDesdeServidor() {
        ...
        if (hiloCliente.getPiso() > Config.piso && hiloCliente.getPiso() != 0) {
            System.out.println("PantallaLujuriaMulti: Detectado cambio de piso a " + hiloCliente.getPiso());
