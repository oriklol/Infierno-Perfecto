...
public class PantallaHistoria implements Screen {
    ...
    public void show() {
        ...
        for (Imagen vineta : vinetas) {
            if (vineta != null) vineta.setSize(Config.ANCHO, Config.ALTO);
        }
    ...
    public void render(float delta) {
        ...
        Render.batch.begin();
        if (indice < vinetas.length && vinetas[indice] != null) {
    ...
    public void dispose() {
        if (vinetas != null) {
            for (int i = 0; i < vinetas.length; i++) {
                if (vinetas[i] != null) {
                    try { vinetas[i].dispose(); } catch (Exception e) { }
                    vinetas[i] = null;
                }
            }
            vinetas = null;
        ...
        if (parrafos != null) {
            for (int i = 0; i < parrafos.length; i++) {
                if (parrafos[i] != null) {
                    try { parrafos[i].dispose(); } catch (Exception e) { }
                    parrafos[i] = null;
                }
            }
            parrafos = null;
